...
class _CoverLetterScreenState extends State<CoverLetterScreen> {
  ...
  bool _inCompactPdfPreview = false;
  ...
  void initState() {
    ...
    timer = Timer.periodic(Duration(milliseconds: 500), (_) async {
      if (_shoudRepaintPdf && !_compactView) {
        await _reloadPdfPreview(await _generateDocumentModel());
        _shoudRepaintPdf = false;
      }
    });
  }
  ...
  void didChangeDependencies() {
    ...
    if (!_compactView && _inCompactPdfPreview) {
      Navigator.of(_compactPdfPreviewContext!).pop();
      _inCompactPdfPreview = false;
  ...
  Future<void> _reloadPdfPreview(CoverLetterModel model) async {
  ...
  void _createCoverLetter() async {
    ...
    try {
      final coverLetter = await _generateDocumentModel();
      if (mounted) {
        await showDialog(
